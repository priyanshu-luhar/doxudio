    ...
    def show_book_details(self, book):
        ...
        if cover_path:
            ...
            try:
                ...
                ctk.CTkLabel(left, text="[No Cover]", width=200, height=300).pack()
        else:
            ctk.CTkLabel(left, text="[No Cover]", width=200, height=300).pack()

        ctk.CTkLabel(left, text=f"Title: {title}", font=self.details_font).pack(pady=5)
        ctk.CTkLabel(left, text=f"Author: {author}", font=self.details_font).pack(pady=5)
        ctk.CTkLabel(left, text=f"Published: {year}", font=self.details_font).pack(pady=5)
        ctk.CTkLabel(left, text=f"Abstract:\n{abstract}", font=self.details_font, wraplength=450, justify="left").pack(pady=10)
        ctk.CTkButton(left, text="Back to Results", font=self.details_font, command=self.back_to_results).pack(pady=20)

        # RIGHT: Reviews
        right = ctk.CTkScrollableFrame(layout)
        right.grid(row=0, column=1, sticky="nsew")

        ctk.CTkLabel(right, text="Reviews", font=self.top_nav_font).pack(pady=(0, 10))
        ...
        if reviews and isinstance(reviews, list):
            for rev in reviews:
                ...
                review_block = ctk.CTkFrame(right)
            ...
            ctk.CTkLabel(right, text="No reviews found.", font=self.default_font).pack()
