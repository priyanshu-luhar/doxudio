...
import requests
import globals
from server_utils import search_book, get_reviews_for_book
...
class DatabaseApp(ctk.CTk):
    ...
    def show_book_details(self, book):
        ...
        main_info_frame = ctk.CTkFrame(self.details_frame)
        main_info_frame.pack(side="left", fill="both", expand=True, padx=20, pady=20)

        reviews_frame = ctk.CTkScrollableFrame(self.details_frame, width=400)
        reviews_frame.pack(side="right", fill="both", padx=20, pady=20)
        ...
        if cover_path:
            ...
            try:
                ...
                ctk.CTkLabel(main_info_frame, image=ctk_img, text="").pack(pady=(10, 10))
            except:
                ctk.CTkLabel(main_info_frame, text="[No Cover]", width=200, height=300).pack()
        else:
            ctk.CTkLabel(main_info_frame, text="[No Cover]", width=200, height=300).pack()

        ctk.CTkLabel(main_info_frame, text=f"Title: {title}", font=self.default_font).pack(pady=5)
        ctk.CTkLabel(main_info_frame, text=f"Author: {author}", font=self.default_font).pack(pady=5)
        ctk.CTkLabel(main_info_frame, text=f"Published: {year}", font=self.default_font).pack(pady=5)
        ctk.CTkLabel(main_info_frame, text=f"Abstract:\n{abstract}", font=self.default_font, wraplength=600, justify="left").pack(pady=10)

        ctk.CTkButton(main_info_frame, text="Back to Results", font=self.default_font, command=self.back_to_results).pack(pady=20)

        # Reviews Section
        ctk.CTkLabel(reviews_frame, text="Reviews", font=self.top_nav_font).pack(pady=(0, 10))
        reviews = get_reviews_for_book(book_id=book.get("book_id"))
        if reviews and isinstance(reviews, list):
            for rev in reviews:
                ...
                review_block = ctk.CTkFrame(reviews_frame)
            ...
            ctk.CTkLabel(reviews_frame, text="No reviews found.", font=self.default_font).pack()
